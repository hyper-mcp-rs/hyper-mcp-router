...
/// text-embedding-005 is a distinct engine with its own endpoint path and the
/// 2048-token-class budgets — and routes end-to-end like the others.
#[tokio::test]
async fn text_embedding_005_uses_own_endpoint_and_routes() {
    let h = Harness::start("text-embedding-005").await;
    let calls = h.embed_calls();
    assert!(
        calls[0]
            .path
            .contains("models/text-embedding-005:batchEmbedContents"),
        "unexpected endpoint path: {}",
        calls[0].path
    );
    assert_eq!(calls[0].api_key.as_deref(), Some("test-gemini-key"));
